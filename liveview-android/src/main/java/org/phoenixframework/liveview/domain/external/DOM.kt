...
object DOM {
    ...
    sealed class MaybeOne(elements: Elements, cssQuery: String, type: String, val count: Int, countWord: String) :
    ...
    fun attribute(@Suppress("UNUSED_PARAMETER") unparsed: String, @Suppress("UNUSED_PARAMETER") key: String): String? =
        null
    ...
    private fun Node.withoutPrettyPrint(): Node = if (this.ownerDocument()?.outputSettings()?.prettyPrint() == false) {
        this
    } else {
        this
            .clone()
    ...
    fun innerHTML(elements: Elements, id: String): Elements = byID(elements, id).let { childNodes(it) }
    ...
    fun deepMerge(target: Map<String, Any>, source: Map<String, Any>): Map<String, Any> = target + source
    ...
    private fun traverseAndAccumulate(element: Element, acc: Elements, predicate: (Element) -> Boolean): Elements {
        var mutAcc = acc
    ...
    fun patchID(id: String, htmlTree: Elements, innerHtml: Elements): Pair<Elements, Set<ComponentID>> {
    ...
    private fun componentIDs(element: Element): Set<ComponentID> {
        val level = attribute(element, PHX_COMPONENT)?.toInt()?.let(::ComponentID)?.let(::setOf).orEmpty()
