...
internal class SnackbarView private constructor(props: Properties) :
    ...
    companion object {
        fun visualsFromNode(node: CoreNodeElement?): Visuals {
            return SnackbarViewFactory.handleAttrs(
...
internal object SnackbarViewFactory : ComposableViewFactory<SnackbarView>() {
    ...
    override fun buildComposableView(
        ...
        pushEvent: PushEvent?,
        scope: Any?
    ): SnackbarView = handleAttrs(attributes, pushEvent, scope).build()

    internal fun handleAttrs(
        attributes: ImmutableList<CoreAttribute>,
        pushEvent: PushEvent?,
        scope: Any?
    ): SnackbarView.Builder = attributes.fold(SnackbarView.Builder()) { builder, attribute ->
        when (attribute.name) {
